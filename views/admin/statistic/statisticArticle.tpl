                    ...
                    <div class="card-title">
                        ...
                        <form class="form-inline">
                            ...
                            <div class="form-group">
                                ...
                                {{$articleSelect:=.getInput.searchArticleId}}
                                {{range $k,$v:=.Article}}
                                    <option value="{{$v.Id}}"
                                    {{if eq $articleSelect $v.Id}}
                                            selected="selected"
                                    {{end}}>
                                    {{$v.Title}}
                                    </option>
                            ...
                            <a href="/adminStatisticArticle" type="button" class="btn btn-danger"
                               style="border-radius: 5px;">撤回</a>
                            <button type="button" class="btn btn-warning" style="border-radius: 5px;"
                                    onclick="deleteData()">删除
                            </button>
                            <a href="/adminStatisticArticleExcel?searchStartDate={{.getInput.searchStartDate}}&searchEndDate
={{.getInput.searchEndDate}}&searchArticleId={{.getInput.searchArticleId}}" type="button" class="btn btn-success"
...
<script>
    layui.use('laypage', function () {
        ...
        laypage.render({
            ...
            , jump: function (obj, first) {
                ...
                //首次不执行
                if (!first) {
                    window.location.href = "/adminStatisticArticle?page=" + obj.curr + "&searchStartDate=" + searchStartDate
                            + "&searchEndDate=" + searchEndDate + "&searchArticleId=" + searchArticleId
                }
            }
        });
    });
    ...
    function deleteData() {
        ...
        var searchArticleId = $('#searchArticleId').val();
        var ajaxActionUrl = '/adminStatisticArticleDelete';
        /* ajax提交 */
        $.ajax({
            //提交数据的类型 POST GET
            type: "POST",
            ...
            data: {
                ...
                searchArticleId: searchArticleId,
            },
            //返回数据的格式
