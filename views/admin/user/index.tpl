            ...
            <div class="card">
                <div class="card-header">
                    <div class="card-title">
                        <a href="/adminUserAdd" type="button" class="btn btn-success">添加账户</a>
                    </div>
                </div>
                <div class="card-body">
                    <div class="panel panel-default">
                        ...
                        <table class="table">
                            ...
                            <tr>
                                ...
                                <td>{{.UpdateAt}}</td>
                                <td>
                                    <a class="btn btn-xs btn-info" href="/adminUserEdit?id={{.Id}}">
                                        编辑
                                    </a>
...
<script type="application/javascript">
    ...
    function updateStatusToOpen(id) {
        var sureUpdateStatusToOpen = confirm("你确定启用用户？");
        if (sureUpdateStatusToOpen === false) {
            alert("请确认后再重新提交");
            return false;
        }
        var ajaxActionUrl = '/adminUserUpdateStatus';
        /* ajax提交 */
        $.ajax({
            //提交数据的类型 POST GET
            type: "POST",
    ...
    function updateStatusToClose(id) {
        var sureUpdateStatusToClose = confirm("你确定禁用用户？");
    ...
    function deleteData(id) {
        var sureDeleteData = confirm("你确定删除用户");
        if (sureDeleteData === false) {
            alert("请确认后再重新提交");
            return false;
        }
        //
        var ajaxActionUrl = '/adminUserDelete';
        /* ajax提交 */
        $.ajax({
            //提交数据的类型 POST GET
            type: "POST",
